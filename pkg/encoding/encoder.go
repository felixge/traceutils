...
type Encoder struct {
	...
	scratch10     [10]byte     // scratch buf for encoding varints
...
func (e *Encoder) Encode(ev *Event) error {
	...
	if ev.Type == EventString {
		...
		if ev.Type == EventStack {
			...
			if e.err = writePaddedVarint(e.w, uint64(e.buf.Len()), e.scratch10[:]); e.err != nil {
				return e.err
			}
		} else {
			// Write the length of the encoded arguments to the e.w
...
func writeVarint(w io.Writer, v uint64, buf []byte) error {
	_ = buf[9]
...
func writePaddedVarint(w io.Writer, v uint64, buf []byte) error {
	_ = buf[9]
	for i := 0; i < 10; i++ {
		if i < 10-1 {
			buf[i] = 0x80 | byte(v)
